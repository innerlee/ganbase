...
if opt.workdir is None:
    opt.workdir = f'samples/began/exp_{datetime.now()}'.replace(' ', '_')
...
def get_optimizerG(lrG):
    if opt.optimizerG == 'adam':
        optimizerG = optim.Adam(netG.parameters(), lr=lrG, betas=(opt.beta1G, 0.9))
...
def get_optimizerD(lrD):
    if opt.optimizerD == 'adam':
        optimizerD = optim.Adam(netD.parameters(), lr=lrD, betas=(opt.beta1D, 0.9))
...
for it in range(1, opt.nIter - 1):
    ...
    for r in range(opt.repeatD):
        ...
        d_loss_real = loss(netD(x_real), x_real)
        d_loss_fake = loss(netD(x_fake), x_fake)
    ...
    for r in range(opt.repeatG):
        ...
        g_loss = loss(netD(x_fake), x_fake)
    ...
    g_d_balance = (opt.gamma * d_loss_real - d_loss_fake).item()
    k_t += opt.lambda_k * g_d_balance
    k_t = max(min(1, k_t), 0)

    measure = d_loss_real.item() + abs(g_d_balance)
